...
void *HelloWorld(void *tidptr)
{
  unsigned long factorial = 1;
  for (int j = 1; j <= *((int *) tidptr); j++) {
    factorial *= j;
  }
  printf("Hello World from thread %d (my factorial is %ld)\n",
      *((int *) tidptr), factorial);
  return NULL;
}
...
int main(int argc, char *argv[])
{
  ...
  int inputVal[nthreads];

  for(int i=0; i<nthreads; ++i){

    t[i] = i;
    inputVal[i]=i;
    rc = pthread_create(&t[i], NULL, HelloWorld, &inputVal[i]);
  ...
  /*
     for(int i=0; i<nthreads; ++i){
     t[i] = i;
     rc = pthread_create(&(t[i]), NULL, HelloWorld, (void *) &i);
     if (rc){
     fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", rc);
     exit(-1);
     }
  ...
  printf("Goodbye World from the main thread\n");
  return(0);
}
